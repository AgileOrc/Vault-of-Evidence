import { ChevronDown, Plus, Search, Globe, Users, ClipboardList, Bug, Pencil, Trash2 } from 'lucide-react'
  ...
  return (
    <div className='space-y-6'>
      <header className='flex flex-wrap items-start justify-between gap-4'>
        <div>
          <h1 className='text-3xl font-semibold'>mycompany.com</h1>
          <p className='mt-1 text-sm opacity-80'>Project description here blablablabla</p>
        </div>
        <button
          type='button'
      ...
      <div className='space-y-4'>
        {worklists.map((item) => (
          <div key={item.name} className={`rounded-2xl px-6 py-4 ${cardBase}`}>
            <div className='flex items-start justify-between gap-4'>
              <div>
                ...
                {item.showActions && (
                  <p className='mt-2 text-xs opacity-80'>2 findings</p>
                )}
              </div>
              <div className='flex items-center gap-3'>
                <span className={`rounded-full px-4 py-1 text-xs font-semibold ${badgeClass(item.status)}`}>
                  {item.status}
                </span>
                {item.showActions && (
                  <div className='flex items-center gap-2'>
                    <button type='button' className='rounded-lg border border-[#27D6FF]/40 p-2'>
